...
public class Storage {
    private static String TAG = TrainActivity.class.getSimpleName(); //added
...
//    }
    ...
    //added
    public void putImage(String cropFileName,String cropFolderName, Mat images, int number){
        String singleFileName= cropFileName;
        File croppath = new File(Environment.getExternalStorageDirectory(), cropFolderName+number); //create folder
        croppath.mkdirs();
//        cropFileName="temp"+i+".jpg";
        File cropFile = new File(croppath,cropFileName); //put photo file into folder
        Boolean bool= null;
        cropFileName= cropFile.toString();
        //write to local storage
        bool = Imgcodecs.imwrite(cropFileName,images); //croped /images is Mat file
        if(bool == true)
            Log.i(TAG,"SUCCESS writing image to external storage "+cropFile.toString());
        else
            Log.i(TAG, "Fail writing image to external storage");

        //write to firebase storage
        //String pathName = croppath.getAbsolutePath();
        StorageReference storageReference;
        Uri uripath = Uri.fromFile(new File(cropFile.toString()));
        if(uripath==null){
            Log.e(TAG,"URI IS NULL");
        }else{
            Log.e(TAG,"image URI PATH "+uripath);
            //storage = FirebaseStorage.getInstance();
            storageReference = FirebaseStorage.getInstance().getReference(cropFolderName); //self declre folder name
            StorageReference ref = storageReference.child(singleFileName); //self declare file name
            ref.putFile(uripath)
                    .addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(@NonNull Exception e) {
                            Log.e(TAG,"NO FIREBASE,croppath ");
                        }
                    }).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                @Override
                public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                    Log.e(TAG,"SUCCESS FIREBASE");
                }
            });
    ...
    public File putXML(final String username) {
        //local storage
        ...
        Log.i(TAG, "SUCCESS writing image to external storage " + file.toString());
        return file;
    }


    public void putFBXML(final String username, File file){
    //write to firebase storage
        StorageReference storageReference;
        Uri uripath = Uri.fromFile(new File(file.toString()));
        if(uripath==null){
            Log.e(TAG,"URI IS NULL");
        }else{
            Log.e(TAG,"xml uri path "+uripath);
            //storage = FirebaseStorage.getInstance();
            storageReference = FirebaseStorage.getInstance().getReference(username); //self declare folder name
            StorageReference ref = storageReference.child("Model XML"); //self declare file name
            ref.putFile(uripath)
                    .addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(@NonNull Exception e) {
                            Log.e(TAG,"NO FIREBASE username"+username);
                        }
                    }).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                @Override
                public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
